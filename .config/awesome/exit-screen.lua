...
-- Appearance
-- icomoon symbols
local icon_font = "icomoon-feather bold 45"
...
local create_button = function(symbol, hover_color, text, command)
    ...
    button:connect_signal("mouse::leave", function ()
        icon.markup = helpers.colorize_text(icon.text, "#FDFDFD")
...
-- Create the buttons
local poweroff = create_button(poweroff_text_icon, "#F37F97", "Poweroff", poweroff_command)
local reboot = create_button(reboot_text_icon, "#5ADECD", "Reboot", reboot_command)
local suspend = create_button(suspend_text_icon, "#F2A272", "Suspend", suspend_command)
local exit = create_button(exit_text_icon, "#8897F4", "Exit", exit_command)
local lock = create_button(lock_text_icon, "#C574DD", "Lock", lock_command)
